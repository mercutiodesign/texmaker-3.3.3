...
/////////////////
    ...
    if (!isGraphic) {
        ...
        for (int j=0; j < text.length(); j++) {
            blockData->code.append(0);
            blockData->misspelled.append(false);
        }
        while (i < text.length()) {
            ch = text.at( i );
            buffer += ch;
...
//                    }
                    ...
                    if(buffer.indexOf("begin{verbatim}") != -1) {
                        state=StateVerbatim;
                    } else if(buffer.indexOf("begin{verbatim*}") != -1) {
                        state=StateVerbatim;
                    } else if(buffer.indexOf("begin{lstlisting}") != -1) {
                        state=StateVerbatim;
                    } else if(buffer.indexOf("begin{gnuplot}") != -1) {
                        state=StateVerbatim;
                    } else if(buffer.indexOf("begin{asy}") != -1) {
                        state=StateGraphicAsy;
                    } else if(buffer.indexOf("begin{tikzpicture}") != -1) {
                        state=StateGraphic;
                    } else if(buffer.indexOf("begin{pspicture}") != -1) {
                        state=StateGraphic;
                    } else if(buffer.indexOf("begin{pspicture*}") != -1) {
                        state=StateGraphic;
                    }
...
//	buffer = QString::null;
            ...
            case StateCommand: {
                ...
                if (rxverb.exactMatch(buffer)) {
                    ...
                    setFormat( i, 1,ColorCommand);
                    state=StateVerbatim;
                } else if (rxlst.exactMatch(buffer)) {
            ...
            case StateGraphic: {
                ...
                if (tmp=='\\') {
                    ...
                    setFormat( i, 1,ColorVerbatim);
                    buffer = QString::null;
                } else if (isWordSeparator(tmp) && tmp!='"' && tmp!='.' && tmp!='-') {
                    blockData->code[i]=1;
                    setFormat( i, 1,ColorVerbatim);
                    buffer = QString::null;
                } else {
